...
def menu(record,r_name):
	...
	if modeA == "1":
		...
		print("1. Guess the day of the week IN 10 SECONDS that corresponds to question! Only the first 3 letter(s)! (Case INSENSITIVE)")
		...
		print("\nSpecial notes: If you play with 1 - 4 digit mode, it will be forced 4-digit format")
		print("\n1-digit = (From Dec - 26 of) -0001 - 0009 \n\n2-digit = 0010 - 0099\n\n3-digit = 0100 - 0999\n\nWhile the rest is proper 4-digit = 1000 - 9999")
			...
			if year > 1:
				...
				if month == 1 or month == 3 or month == 5 or month == 7 or month == 8 or month == 10 or month == 12:
					...
					if (y400%4 == 0 and y400%100 == 0 and y400%400 == 0):
						...
						d = rand(1,28)
				print("%s - %s - %s" % (y1,c.month_name[month],d))
				try:
				...
				if y == -1:
					month = 12
					d = rand(26,31)
					print("%05d - %s - %s" % (y,c.month_name[month],d))
					try:
						ans = inputimeout(prompt="\n\nAnswer -->  ",timeout=10)
					...
					if month == 1 or month == 3 or month == 5 or month == 7 or month == 8 or month == 10 or month == 12:
						...
						if (y%4 == 0 and y%100 == 0 and y%400 == 0):
							...
							d = rand(1,28)
					print("%04d - %s - %s" % (y,c.month_name[month],d))
					try:
						ans = inputimeout(prompt="\n\nAnswer -->  ",timeout=10)
						if ans.lower() == c.day_abbr[c.weekday(y,month,d)].lower():
							pts = pts + year
		...
		print("Average point(s) this session = %s (%s/%s)\n\n" % (total//len(record),total%(len(record)),len(record)))
		x13 = input("\n\nEnter to continue to main menu!")
		clear()
		menu(record,r_name)
...
try:
	...
	try:
		clear()
		print("SEE YOU SOON!!\n\n")
		print("\n\n\n©2023 GREGORIAN GAME by AditD33")
		time.sleep(5)
		clear()
		sys.exit()
	except KeyboardInterrupt:
